...
holder_did=$(cat holder/did.json | jq '.id' -r)
...
signature=$(echo -n "${jwt_header}.${payload}" | openssl dgst -sha256 -binary -sign holder/private-key.pem | base64 -w0 | sed s/\+/-/g | sed 's/\//_/g' | sed -E s/=+$//)
